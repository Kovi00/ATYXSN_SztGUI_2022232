    ...
    internal class CrudService
    {
        ...
        public void Create<T>()
        {
            var properties = typeof(T).GetProperties()
                .Where(prop => prop.GetAccessors().All(a => !a.IsVirtual) && prop.Name != "Id");
            ...
            foreach (var property in properties)
            {
                ...
                {
                    property.SetValue(instance, char.Parse(input));
                }
                else
                {
        ...
        public void Update<T>()
        {
            ...
            var properties = typeof(T).GetProperties()
                .Where(p => p.GetAccessors().All(a => !a.IsVirtual) && p.Name != "Id");

            foreach (var property in properties)
            {
                Console.Write($"{property.Name} [old: {property.GetValue(instance)}] = ");
                string input = Console.ReadLine();
                if (property.PropertyType == typeof(int))
                {
                    property.SetValue(instance, int.Parse(input));
